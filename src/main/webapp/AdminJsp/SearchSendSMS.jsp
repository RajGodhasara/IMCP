<%-- 
    Document   : SearchSendSMS
    Created on : Apr 7, 2017, 11:50:06 AM
    Author     : GOPIRAJ
--%>
...
<jsp:include page="../IncludeResourse/DataTables.jsp"></jsp:include>
        <script type="text/javascript">
            ...
            function deleteSelectedCourse() {
                var checkboxx = document.getElementsByName('listSelectedSendSms');
                var listSelectedSendSms = "";
                for (var i = 0; i < checkboxx.length; i++) {
                    if (checkboxx[i].checked) {
                        listSelectedSendSms = listSelectedSendSms + checkboxx[i].value + ":";
                    }
                }
                    ...
                    buttons:
                            {
                                "Delete data": function() {
                                    $(this).dialog("close");
                                    location.href = "admin_delete_multiple_sendsms?listSelectedSendSms=" + listSelectedSendSms;
                                },
                                Cancel: function() {
                                    $(this).dialog("close");
                                }
                        ...
                        <table id="example" class="display" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                <c:forEach items="${list}" var="list">
                                    <tr>
                                        <td>${list.sentSmsId}</td>
                                        <td>${list.person.firstName}</td>
                                        <td>${list.enquiry.firstName}</td>
                                        <td>${list.preText.title}</td>
                                        <td><a class="edit" href="">Edit</a></td>
                                        <td><input type="checkbox" name="listSelectedSendSms"  id="listSelectedSendSms"
                                                   value="${list.sentSmsId}" style="width:30px;" /></td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                        </table>
...
<div style="margin-top: 80px; margin-left: 100px;">
    <input type="button" id="buttondelete"  onclick="deleteSelectedCourse()" value="Delete Selected SentSMS" class="btn btn-primary"/>
    </div>
